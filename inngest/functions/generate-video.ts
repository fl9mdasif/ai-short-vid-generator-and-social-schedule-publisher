    ...
    async ({ event, step }) => {
        ...
        await step.run("publish-video-into-social", async () => {
            ...
            if (seriesData.platform?.includes('youtube')) {
                await step.run("publish-to-youtube", async () => {
                    if (!finalVideoUrl) {
                        throw new Error("Cannot publish to YouTube: No final video URL available");
                    }

                    console.log(`[YouTube] Starting publish for series: ${seriesData.series_name}`);

                    try {
                        const { getValidYoutubeToken, uploadVideoToYoutube } = await import("@/lib/youtube");

                        // Get valid token (handles refresh)
                        const accessToken = await getValidYoutubeToken(seriesData.user_clerk_id);

                        // Upload video
                        // Defaulting to 'private' for safety during initial testing. 
                        // You can change to 'public' when ready.
                        const result = await uploadVideoToYoutube(
                            accessToken,
                            finalVideoUrl,
                            `${seriesData.series_name} #shorts`, // Title
                            `${seriesData.series_description}\n\nGenerated by VidMaxx. #ai #shorts`, // Description
                            'private'
                        );

                        console.log(`[YouTube] Published successfully! Video ID: ${result.id}`);
                        return { platform: 'youtube', videoId: result.id, status: 'published' };
                    } catch (error: any) {
                        console.error("[YouTube] Publish failed:", error);
                        // Don't fail the whole workflow, just log/return error
                        return { platform: 'youtube', status: 'failed', error: error.message };
                    }
                });
            }
